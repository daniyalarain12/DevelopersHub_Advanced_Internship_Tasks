...
Objective: Predict whether a client will subscribe to a bank term deposit based on direct marketing campaigns. Methodology:
...
Objective: Analyze loan origination data to predict credit risk, specifically the likelihood of a 12-month default (default_12m). Methodology:
...
3. Time Series.ipynb
Objective: Forecast metrics derived from household power consumption data using advanced time-series techniques. Methodology:
...
4. app.py
Objective: A dynamic, interactive executive dashboard for global sales and profitability tracking. Methodology:
...
plotly
How to Use
Ensure all dependencies are installed via pip install -r requirements.txt (or install manually).
For the Notebooks (.ipynb): Launch Jupyter Notebook or JupyterLab, open the desired file, and execute the cells sequentially.
For the Dashboard (app.py): Run the command streamlit run app.py in your terminal to launch the web application locally.
